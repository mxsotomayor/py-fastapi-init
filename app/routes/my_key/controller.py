...
@router.post(f"{base_path}/confirm", response_model=bool, status_code=status.HTTP_200_OK, tags=["My Keys"])
def validate_key(key_confirm: MyKeyConfirm, db: Session = Depends(get_db)):
    device_service = MyKeysService()
    try:
        db_device = device_service.verify_key(db=db, confirm_data=key_confirm)
        return db_device
    except Exception as e:
        # Generic error handling for unexpected issues
        raise HTTPException(
